...
def categorize_dish(dish_name, dish_ingredients):
    ...
    if set(dish_ingredients).issubset(VEGAN): type_food = "VEGAN"
    if set(dish_ingredients).issubset(VEGETARIAN): type_food = "VEGETARIAN"
    if set(dish_ingredients).issubset(KETO): type_food = "KETO"
    if set(dish_ingredients).issubset(PALEO): type_food = "PALEO"
    if set(dish_ingredients).issubset(OMNIVORE): type_food = "OMNIVORE"
    
    return f"{dish_name}: {type_food}"
...
def compile_ingredients(dishes):
    ...
    # list_ingredients = {}
    # for dish in dishes:
    #     list_ingredients = set(list_ingredients).union(dish)
...
def singleton_ingredients(dishes, intersection):
    ...
    singletons = (set(dish).difference(intersection) for dish in dishes)
